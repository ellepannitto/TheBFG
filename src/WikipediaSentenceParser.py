...
class WikipediaSentenceParser:
	...
	def parse_sent (self, raw_sentence):
		...
		#~ print ("{}: parse sentence {}".format(os.getpid(), raw_sentence))
		...
		for i, token in sentence.items():
			if token.rel == "prt" or token.enhanced_rel == "prt":
